<?php
/*
Plugin Name:    UpFront Lifesaver
Plugin URI:     https://upfrontunlimited/plugins/upfront-lifesaver
Description:    UpFront Lifesaver plugin allows convert Headway or Blox Templates to UpFront Unlimited Templates. Based on the original plugin hw-to-bt from Johnathan.PRO.
Version:        1.0.14
Author:         UpFront Unlimited Team
Author URI:     https://www.upfrontunlimited.com/
License:        GPL2
License URI:    https://www.gnu.org/licenses/gpl-2.0.html
Text Domain:    upfront-lifesaver
Domain Path:    /languages
Network: false


UpFront Services plugin is free software: you can redistribute it and/or modify
it under the terms of the GNU General Public License as published by
the Free Software Foundation, either version 2 of the License, or
any later version.
 
UpFront Services plugin is distributed in the hope that it will be useful,
but WITHOUT ANY WARRANTY; without even the implied warranty of
MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the
GNU General Public License for more details.
 
You should have received a copy of the GNU General Public License
along with UpFront Services plugin. If not, see https://www.gnu.org/licenses/gpl-2.0.html.
*/
...
class Lifesaver extends UpFrontLifesaver\helpers\Plugin {
    ...
    public function __construct($args = false) {
        ...
        $_SESSION['upfront-lifesaver-source'] = $this->source;
        ...
        $this->actions = array(
            ...
            'admin_notices'         =>  false,            
            'wp_async_lifesaver_json'  =>  'setUpFrontUnlimited'
        );
        ...
        $this->upfront_dir    = WP_CONTENT_DIR . '/themes/upfront';
        $this->source_exist = file_exists($this->source_dir);
        $this->upfront_exist  = file_exists($this->upfront_dir);
        $this->template     = '';
        $this->stylesheet   = '';
        ...
        new UpFrontLifesaver\helpers\json();
    ...
    private function testBeforeGo(){
        ...
        if($_POST && $_SERVER['QUERY_STRING'] == 'page=upfront-lifesaver'){

            $info_link = '<a href="https://www.upfrontunlimited.com/blog/2018/website-migration-from-headway-themes-3-8-x-bloxtheme-1-0-x-to-upfront-unlimited-theme-builder/" target="_blank">More information</a>';
    ...
    private function getSource() {
        ...
        if($this->source == 'headway'){
            ...
            ob_start();
            $filename   = UpFrontLifesaver\helpers\HeadwayDataPortability::export_skin($info);
            $skin       = ob_get_clean();
            ...
            ob_start();
            $filename   = UpFrontLifesaver\helpers\BloxDataPortability::export_skin($info);
            $skin       = ob_get_clean();
        }
        return $skin;
    ...
    private function converttoUpFrontUnlimited($json) {
        
        if($this->source == 'headway'){

            $json = preg_replace('/(hw)/', 'pu', $json);
            $json = preg_replace('/(headway\_)/', 'upfront_', $json);

        }elseif ($this->source == 'bloxtheme') {

            $json = preg_replace('/(bt)/', 'pu', $json);
            $json = preg_replace('/(bloxtheme\_)/', 'upfront_', $json);
        ...
        if($this->source == 'headway'){
            ...
            $data_pu_blocks     = preg_replace('/(\-headway\-)/', '-upfront-', $data_pu_blocks);

        }elseif ($this->source == 'bloxtheme'){

            $data_pu_blocks     = preg_replace('/(\-bloxtheme\-)/', '-upfront-', $data_pu_blocks);
        }
    ...
    public function setUpFrontUnlimited($include_widgets) {


        if(! class_exists('UpFront')) {
            require_once(dirname(__FILE__) . '/helpers/upfront/functions.php');
            require_once($this->upfront_dir . '/library/common/application.php');
        }
        ...
        if(file_exists($this->skin_path)) {
            ...
            $json = UpFrontVisualEditorAJAX::replace_imported_images_variables($json);
            UpFrontDataPortability::install_skin( json_decode($json, true));
        ...
        if(! empty($include_widgets)) {
            ...
            // migrate HW/Blox widgets to UpFront
            switch($this->template) {
                
                case 'upfront':// if UpFront in use
                ...
                case 'bloxtheme':// if Bloxtheme in use
                    ...
                    update_option('theme_mods_upfront', $theme_mods);
                    break;
        ...
        ob_start();
        UpFrontVisualEditorAJAX::secure_method_switch_skin();
        ob_get_clean();
    ...
    public function plugins_loaded() {
        ...
        if(isset($_REQUEST['UpFrontLifesaver'])) {

            if(wp_verify_nonce($_REQUEST['nonce'], 'UpFrontLifesaver_nonce') !== false) {


                $this->template     = get_option('template', '');                
                if($this->template != 'headway' && $this->template != 'bloxtheme') {
                    update_option('template', 'upfront');
                }

                $this->stylesheet = get_option('stylesheet', '');
                if($this->stylesheet != 'headway' && $this->stylesheet != 'bloxtheme') {
                    update_option('stylesheet', 'upfront');
                }

            } else {
                throw new Exception('UpFrontLifesaver nonce invalid');
            }
            ...
            if(wp_verify_nonce($_REQUEST['nonce'], 'lifesaver_skin_nonce') !== false) {
                ...
                $this->template     = get_option('template', '');
                if($this->template != 'upfront') {
                    update_option('template', 'upfront');
                }

                $this->stylesheet = get_option('stylesheet', '');
                if($this->stylesheet != 'upfront') {
                    update_option('stylesheet', 'upfront');
                }
    ...
    public function after_setup_theme() {
        ...
        if(isset($_REQUEST['UpFrontLifesaver'])) {
            ...
            if(wp_verify_nonce($_REQUEST['nonce'], 'UpFrontLifesaver_nonce') !== false) {
                ...
                $skin = $this->getSource();                
                $json = $this->converttoUpFrontUnlimited($skin);                
                ...
                if(! empty($json)) {
                    ...
                    if(isset($frags[1])) {
                        ...
                        $query['UpFrontLifesaver-convert'] = 'complete';
    ...
    public function admin_notices() {
        ...
        if(isset($_GET['UpFrontLifesaver-convert']) && $_GET['UpFrontLifesaver-convert'] == 'complete') {
            $this->render_msg(ucfirst($this->source) . ' to UpFront Unlimited conversion completed. You can safely uninstall this plugin.');
        }
    ...
    public function Settings() {
        ...
        if(! $this->upfront_exist) {
            $this->render_err('UpFront Unlimited does not exist! Please install the latest version.');
            die();
        }

        if($_GET['UpFrontLifesaver-convert']!='complete'){            
            $this->render_msg('Make a full backup before start.');
            if( empty($this->source) )
                $this->render_msg('Neither Headway nor Blox was detected');
            else
        ...
        $this->render('settings', array(
            ...
            'upfront_exist'   =>  $this->upfront_exist,
            'nonce'         =>  wp_create_nonce('UpFrontLifesaver_nonce'),
            'templates'     =>  $templates
        ));
    }
 ...
 *
 */
function upfront_lifesaver_activate(){
    ...
    foreach ($tables as $table_name => $table_structure) {        
        ...
        if($wpdb->get_var("SHOW TABLES LIKE '$table_name'") != $table_name) {             
            ...
            $sql = get_upfront_db_structures($table_name, $table_structure);            
            dbDelta( $sql );
...
register_activation_hook( __FILE__, 'upfront_lifesaver_activate');



function get_upfront_db_structures($table_name, $table_structure){
